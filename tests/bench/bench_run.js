...
const TAG = `${SET}_${ITERS}` + (Q.has('classic') ? '_classic' : '')
  ...
  + (Q.has('classicsolve') ? '_cs' : '') + (Q.get('featres') ? `_fr${Q.get('featres')}` : '') + (Q.get('feats') ? `_nf${Q.get('feats')}` : '') + (Q.get('octave') ? `_oc${Q.get('octave')}` : '') + (Q.get('peak') ? `_pk${Q.get('peak')}` : '') + (Q.get('solve') ? `_sv${Q.get('solve')}` : '') + (Q.get('baratio') != null ? `_br${Q.get('baratio')}` : '')
...
try {
  ...
  const ses = window.__ses = createSession({
    ...
    sfm: {
      ...
      ...(Q.get('baratio') != null ? { interimBARatio: +Q.get('baratio') } : {}),   // interim-BA cadence (0 = every 6 registrations, the pre-09-09 fixed cadence)
    },
