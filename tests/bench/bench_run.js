...
const TAG = `${SET}_${ITERS}` + (Q.has('classic') ? '_classic' : '')
  ...
  + (Q.get('masks') ? '_msk' : '') + (Q.get('hullkeep') ? `_hk${Q.get('hullkeep')}` : '') + (Q.get('hullres') ? `_hr${Q.get('hullres')}` : '') + (Q.get('covw') ? `_cw${Q.get('covw')}` : '') + (Q.get('covs') ? `_cs${Q.get('covs')}` : '') + (Q.get('maskcut') ? `_mc${Q.get('maskcut')}` : '') + (Q.get('maskbg') ? `_mb${Q.get('maskbg')}` : '')
