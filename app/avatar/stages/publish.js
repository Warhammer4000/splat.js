...
export async function run(ctx, manifest, hooks) {
  ...
  const clicked = await new Promise((resolve) => {
    ...
    body.innerHTML = `<div class="upcard-row"><p class="fine">Splat ${fmtMB(ply.size)} · binding ${fmtMB(bin.byteLength)} · rig ${fmtMB(glb.byteLength)}. ${hasToken() ? '' : 'You will be asked to sign in to arrival.space.'}</p>
  ...
  keys.splat = await up(ply, `${name}.ply`, 'splat');
  ...
  return { avatarId: d.id, url: d.url, assigned: !!d.assigned, note: `avatar ${d.id}` };
}
